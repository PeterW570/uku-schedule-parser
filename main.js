...
const teamAliases = require('./team-aliases.json');
            ...
            for (let division in parsedRes.allGames) {
                ...
                parsedRes.allGames[division].forEach(({ timestamp, pool, teams }) => {
                    ...
                    teams.forEach(({ team, score, seed }) => {
                        ...
                        if (pool) {
                            let seedMatch = parsedRes.initialSeedings[division].find(({ poolSeed }) => poolSeed === seed);
                            seed = seedMatch.seed;
                        }
                ...
                (parsedRes.finalPlacings[division] || []).forEach(({ team, seed }) => {
                    ...
                    databaseTables.tournament_placings.push({
                        ...
                        initial_seed: Number(parsedRes.initialSeedings[division].find(({ team: t }) => t === team).seed),
                    });
                });
            }
